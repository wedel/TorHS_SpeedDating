...
pids=""
for PATH_ALG in 'hs_tor' 'hs_short_tor'
do

  echo "Will simulate for $PATH_ALG ($FULLBW_STR)."

  SIM_OUT_DIR=$BASE_DIR/out/simulate/final_bw_alloc/$FULLBW_STR/$PATH_ALG
  ANA_OUT_DIR=$BASE_DIR/out/analyze/final_bw_alloc/$FULLBW_STR/$PATH_ALG

  mkdir -p $BASE_DIR/out/simulate/final_bw_alloc
  mkdir -p $BASE_DIR/out/analyze/final_bw_alloc
  mkdir -p $BASE_DIR/out/simulate/final_bw_alloc/$FULLBW_STR/
  mkdir -p $BASE_DIR/out/analyze/final_bw_alloc/$FULLBW_STR/
  mkdir -p $SIM_OUT_DIR
  mkdir -p $ANA_OUT_DIR
  ...
  do
    ...
    (time python pathsim.py simulate --nsf_dir $NSF_DIR\
     ...
     --num_adv_middles $NUM_ADV_MIDDLES --adv_middle_cons_bw $ADV_MIDDLE_CONS\
     --loglevel $LOGLEVEL $PATH_ALG)\
     2> $SIM_OUT_DIR/$RATIO/simulate.$EXP_NAME.$NUM_SAMPLES-samples.time\
     1> $SIM_OUT_DIR/$RATIO/simulate.$EXP_NAME.$NUM_SAMPLES-samples.out &
    pids="$pids $!"
